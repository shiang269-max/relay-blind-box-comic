    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex flex-col items-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="flex items-center gap-3 mb-6 mt-2">
          ...
          <div>
            <h1 className="text-white font-black text-xl">歷史漫畫</h1>
            <p className="text-slate-400 text-xs">已保存的接力作品</p>
          </div>
        </div>

        {loading && (
          <div className="text-center py-16 text-slate-400 animate-pulse">載入中...</div>
        )}
        ...
        {!loading && comics.length > 0 && (
          <div className="flex flex-col gap-3">
            {comics.map(comic => (
              <div
                key={comic.id}
                className="bg-slate-800/60 rounded-2xl p-4 border border-slate-700/50 flex items-center justify-between gap-3"
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-slate-500 text-xs">{formatDate(comic.createdAt)}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-sm font-semibold px-3 py-1.5 rounded-xl border border-red-500/20 transition-all disabled:opacity-40"
                  >
                    <Trash2 size={13} />
                    刪除
                  </button>
