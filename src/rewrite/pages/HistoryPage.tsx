...
export default function HistoryPage({ onBack, onOpen }: HistoryPageProps) {
  ...
  const filteredComics = useMemo(() => {
    ...
    return comics.filter((comic) =>
      comic.title.toLocaleLowerCase().includes(keyword),
    );
  }, [comics, query]);

  return (
    <div className="relative min-h-[100svh] overflow-hidden text-white">
      <GameAtmosphere map={selectedMap} round={30} />
      ...
      <main className="relative z-10 mx-auto flex min-h-[100svh] w-full max-w-3xl flex-col px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))] sm:px-4">
        <header className="mb-5 flex items-center justify-between gap-3 rounded-3xl border border-white/15 bg-slate-950/35 px-3 py-3 shadow-2xl backdrop-blur-xl sm:px-4">
          <button
            onClick={onBack}
            className="min-h-11 rounded-2xl px-3 text-sm text-white/75 active:scale-[0.98]"
          >
            ← 返回
          </button>
        ...
        <section className="mb-4 rounded-3xl border border-white/15 bg-slate-950/35 p-3 shadow-2xl backdrop-blur-xl sm:p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs text-white/55">
                {loading ? "正在整理你的世界..." : `目前收藏 ${comics.length} 部接力漫畫`}
              </div>
            </div>
            <div className="rounded-2xl border border-white/15 bg-white/10 px-3 py-2 text-right">
              <div className="text-lg font-black">{comics.length}</div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 rounded-2xl bg-black/20 p-1.5">
            <button
              ...
              className={`min-h-11 rounded-xl text-sm font-bold transition active:scale-[0.98] ${selectedMap === "earth" ? "bg-sky-400 text-slate-950 shadow-lg" : "text-white/65"}`}
            >
              地球檔案
            </button>
            <button
              onClick={() => setSelectedMap("space")}
              className={`min-h-11 rounded-xl text-sm font-bold transition active:scale-[0.98] ${selectedMap === "space" ? "bg-violet-400 text-slate-950 shadow-lg" : "text-white/65"}`}
            >
              宇宙檔案
            </button>
          </div>
          ...
          <input
            ...
            placeholder="搜尋漫畫名稱"
            className="mt-3 min-h-12 w-full rounded-2xl border border-white/10 bg-black/25 px-4 text-base outline-none placeholder:text-white/35 focus:border-white/30"
          />
        </section>

        <section className="flex-1 pb-4">
          {loading && (
            <div className="rounded-3xl border border-white/10 bg-slate-950/35 p-10 text-center text-sm text-white/55 backdrop-blur-xl">
          ...
          {!loading && error && (
            <div className="rounded-3xl border border-red-300/20 bg-red-950/30 p-8 text-center text-sm text-red-100 backdrop-blur-xl">
              {error}
            </div>
          )}

          {!loading && !error && filteredComics.length === 0 && (
            <div className="rounded-3xl border border-white/10 bg-slate-950/35 p-10 text-center backdrop-blur-xl">
              <div className="text-lg font-bold">{comics.length === 0 ? "世界還沒有留下作品" : "找不到符合的作品"}</div>
              <p className="mt-2 text-sm text-white/55">
                {comics.length === 0 ? "完成一場 30 回合接力後，漫畫會出現在這裡。" : "試試不同的漫畫名稱。"}
              </p>
            </div>
          )}
              ...
              {filteredComics.map((comic) => {
                ...
                return (
                  <button
                    ...
                    className="group overflow-hidden rounded-3xl border border-white/15 bg-slate-950/45 text-left shadow-xl backdrop-blur-xl transition active:scale-[0.985] sm:hover:-translate-y-1 sm:hover:border-white/30"
                  >
                    <div className="relative aspect-[3/4] overflow-hidden bg-black/30">
                      ...
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/30 to-transparent p-3 pt-10">
                        <div className="flex items-end justify-between gap-2 text-xs text-white/75">
                    ...
                    <div className="p-4">
                      <div className="line-clamp-1 font-black">{comic.title}</div>
                      <div className="mt-1 text-xs text-white/50">{formatCreatedAt(comic.createdAt)}</div>
                    </div>
                  </button>
                );
              })}
