...
function toFirebaseErrorMessage(error: unknown): string { if (error instanceof Error) { if (error.message.includes("permission_denied")) return "Firebase 權限被拒絕，請檢查 Realtime Database Rules。"; if (error.message.includes("timeout")) return "連線逾時，請確認網路後重新加入房間。"; return `Firebase 連線失敗：${error.message}`; } return "Firebase 連線失敗。"; }
...
export function useRoom(session: RoomSession) {
  ...
  const submit = useCallback(async (pageDataUrl: string, score = 0): Promise<boolean> => { const gameId = game?.gameId; if (!gameId) return false; return submitRound(session.roomId, gameId, session.playerId, pageDataUrl, score); }, [game?.gameId, session.playerId, session.roomId]);
