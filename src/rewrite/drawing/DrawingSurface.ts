...
export class DrawingSurface {
  ...
  eventToScreen(event: PointerEvent | WheelEvent): Point {
    ...
    return { x: (event.clientX - rect.left) * this.cssWidth / Math.max(1, rect.width), y: (event.clientY - rect.top) * this.cssHeight / Math.max(1, rect.height) };
  }
  ...
  startStroke(point: Point, brush: Brush): boolean {
    ...
    this.lastPoint = point; this.drawDot(point, brush); this.render(); return true;
  }

  continueStroke(point: Point, brush: Brush): void {
    if (!this.lastPoint) return;
    this.drawSegment(this.lastPoint, point, brush); this.lastPoint = point; this.render();
  }
  ...
  redraw(strokes: readonly Stroke[]): void {
    this.endStroke(); this.strokeContext.clearRect(0, 0, this.options.worldWidth, this.options.worldHeight);
  ...
  async loadImage(source: string): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => { image.onload = () => resolve(); image.onerror = () => reject(new Error("上一頁圖片載入失敗")); image.src = source; });
    this.endStroke(); this.baseContext.clearRect(0, 0, this.options.worldWidth, this.options.worldHeight);
    this.baseContext.drawImage(image, 0, 0, this.options.worldWidth, this.options.worldHeight); this.render();
  }
  ...
  render(): void {
    ...
    ctx.drawImage(this.worldBackgroundCanvas, 0, 0); ctx.drawImage(this.baseCanvas, 0, 0); ctx.drawImage(this.strokeCanvas, 0, 0);
    ctx.setTransform(1, 0, 0, 1, 0, 0);
  }

  /** 接力快照保持與標準畫布相同的原生解析度，避免下一位玩家放大後失去細節。 */
  exportPng(): string {
    this.endStroke();
    ...
    const ctx = output.getContext("2d"); if (!ctx) throw new Error("無法建立輸出畫布");
    ctx.clearRect(0, 0, width, height); ctx.drawImage(this.baseCanvas, 0, 0, width, height); ctx.drawImage(this.strokeCanvas, 0, 0, width, height);
    return output.toDataURL("image/png");
  }

  private createWorldCanvas(): HTMLCanvasElement {
    const canvas = document.createElement("canvas"); canvas.width = this.options.worldWidth; canvas.height = this.options.worldHeight; return canvas;
  }

  private paintWorldBackground(): void {
    const ctx = this.worldBackgroundContext; const { worldWidth: width, worldHeight: height, map, time } = this.options;
    ctx.clearRect(0, 0, width, height); const colors = this.getWorldColors();
    const sky = ctx.createLinearGradient(0, 0, width, height); sky.addColorStop(0, colors.top); sky.addColorStop(0.45, colors.middle); sky.addColorStop(1, colors.bottom); ctx.fillStyle = sky; ctx.fillRect(0, 0, width, height);
    if (time === "dusk") { const haze = ctx.createRadialGradient(width * 0.5, height * 0.42, 0, width * 0.5, height * 0.42, Math.max(width, height) * 0.7); haze.addColorStop(0, "rgba(255,240,210,0.38)"); haze.addColorStop(1, "rgba(255,170,100,0)"); ctx.fillStyle = haze; ctx.fillRect(0, 0, width, height); }
    this.paintStars(ctx, width, height, map === "space" || time === "night" ? (map === "space" ? 260 : 140) : 90);
    if (map === "earth") {
      ...
      planet.addColorStop(0, time === "night" ? "rgba(90,150,210,0.62)" : "rgba(180,235,255,0.78)"); planet.addColorStop(0.28, time === "dusk" ? "rgba(22,110,125,0.95)" : "rgba(16,125,150,0.96)"); planet.addColorStop(0.66, "rgba(8,48,73,0.98)"); planet.addColorStop(1, "rgba(2,6,23,1)"); ctx.fillStyle = planet; ctx.beginPath(); ctx.arc(centerX, centerY, radius, 0, Math.PI * 2); ctx.fill();
    } else {
      const radius = Math.max(width, height) * 0.22, centerX = width * 0.77, centerY = height * 0.14;
      const planet = ctx.createRadialGradient(centerX - radius * 0.25, centerY - radius * 0.3, radius * 0.02, centerX, centerY, radius);
      planet.addColorStop(0, "#e0f2fe"); planet.addColorStop(0.1, "#38bdf8"); planet.addColorStop(0.45, "#0369a1"); planet.addColorStop(1, "#020617"); ctx.fillStyle = planet; ctx.beginPath(); ctx.arc(centerX, centerY, radius, 0, Math.PI * 2); ctx.fill();
    }
    const vignette = ctx.createRadialGradient(width * 0.5, height * 0.5, Math.min(width, height) * 0.1, width * 0.5, height * 0.5, Math.max(width, height) * 0.8); vignette.addColorStop(0, "rgba(0,0,0,0)"); vignette.addColorStop(1, "rgba(2,6,23,0.34)"); ctx.fillStyle = vignette; ctx.fillRect(0, 0, width, height);
  }

  private getWorldColors(): { top: string; middle: string; bottom: string } {
    const { map, time } = this.options;
    if (map === "space") { if (time === "day") return { top: "#0f274f", middle: "#08152f", bottom: "#030712" }; if (time === "dusk") return { top: "#4c1d5f", middle: "#21123d", bottom: "#090611" }; return { top: "#111827", middle: "#030712", bottom: "#000000" }; }
    if (time === "day") return { top: "#7dd3fc", middle: "#38bdf8", bottom: "#172554" }; if (time === "dusk") return { top: "#fef3c7", middle: "#fb7185", bottom: "#312e81" }; return { top: "#172554", middle: "#1e1b4b", bottom: "#020617" };
  }

  private paintStars(ctx: CanvasRenderingContext2D, width: number, height: number, count: number): void {
    let seed = this.options.map === "space" ? 9137 : 4219; const random = () => { seed = (seed * 1664525 + 1013904223) >>> 0; return seed / 0xffffffff; };
    ctx.save(); for (let index = 0; index < count; index += 1) { const x = random() * width, y = random() * height, radius = 0.7 + random() * 2.2, alpha = 0.18 + random() * 0.7; ctx.fillStyle = `rgba(255,255,255,${alpha})`; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); } ctx.restore();
  }
