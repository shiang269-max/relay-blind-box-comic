...
import { Check, Eraser, Hand, Move, Palette, PenLine, Trash2, Undo2, ZoomIn } from "lucide-react";
...
const BRUSH_PRESETS = [3, 6, 12, 24, 40, 64];
...
export default function DrawingScreen({ mode, roomId, pageIndex, round, playerCount, map, playerName, previousPage, onSubmit }: DrawingScreenProps) {
  ...
  const resize = useCallback(() => {
    ...
    if (element && surface) surface.resize(element.clientWidth, element.clientHeight);
  }, []);
  ...
  const saveSnapshotNow = useCallback(async () => {
    ...
    try { await lifecycle.saveSnapshot(); setAutosaveState("saved"); }
      ...
      if (autosaveQueuedRef.current) { autosaveQueuedRef.current = false; void saveSnapshotNow(); }
    }
  }, []);
  ...
  const scheduleAutosave = useCallback(() => {
    ...
    autosaveTimerRef.current = window.setTimeout(() => { autosaveTimerRef.current = null; void saveSnapshotNow(); }, 500);
  }, [saveSnapshotNow]);
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    let cancelled = false;
    setLoadingDrawing(true); setAutosaveState("idle"); setSubmitError(null);
    ...
    surfaceRef.current = surface; sessionRef.current = session; lifecycleRef.current = lifecycle;
    resize();
    ...
    return () => {
      cancelled = true;
      if (autosaveTimerRef.current !== null) window.clearTimeout(autosaveTimerRef.current);
      observer.disconnect(); session.end();
  ...
  const { handlePointerDown, handlePointerMove, finishPointer, handleWheel } = useDrawingInteraction({
    surfaceRef, sessionRef, brush, moveMode, onStrokeEnd: scheduleAutosave, onInteractionChange: setInteraction,
  });
  ...
  const modeLabel = interaction === "pinching" ? "雙指縮放／拖曳" : interaction === "moving" || moveMode ? "移動畫布" : interaction === "eraser" || eraser ? "橡皮擦" : "畫筆";
  const ModeIcon = interaction === "pinching" ? ZoomIn : interaction === "moving" || moveMode ? Hand : interaction === "eraser" || eraser ? Eraser : PenLine;
  ...
  return (
    <div className="relative flex w-screen flex-col overflow-hidden text-white" style={{ height: "var(--app-height, 100svh)", paddingTop: "env(safe-area-inset-top)", paddingBottom: "env(safe-area-inset-bottom)" }}>
      ...
      <main ref={containerRef} className="relative z-10 min-h-0 flex-1 overflow-hidden">
        ...
        <div className="pointer-events-none absolute right-3 top-3 rounded-xl bg-slate-950/55 px-2 py-1 text-[10px] text-white/70">單指作畫 · 雙指縮放</div>
      ...
      <section className="relative z-10 shrink-0 border-t border-white/10 bg-slate-950/45 text-white backdrop-blur-xl"><div className="flex min-h-14 items-center gap-2 overflow-x-auto px-3 py-2 [-webkit-overflow-scrolling:touch]">
        ...
        <div className="flex min-h-11 shrink-0 items-center gap-2 rounded-xl bg-white/10 px-2"><input type="range" min="1" max="64" step="1" value={size} onChange={(event) => setSize(Number(event.target.value))} className="w-24 accent-cyan-300" aria-label="筆刷粗細" /><input type="number" min="1" max="64" value={size} onChange={(event) => { const value = Number(event.target.value); if (Number.isFinite(value)) setSize(Math.max(1, Math.min(64, Math.round(value)))); }} className="w-12 rounded-lg bg-slate-950/70 px-1 py-1 text-center text-xs font-bold outline-none" inputMode="numeric" aria-label="直接輸入筆刷粗細" /></div>
        <div className="flex shrink-0 gap-1">{BRUSH_PRESETS.map((preset) => <button key={preset} onClick={() => setSize(preset)} className={`min-h-9 min-w-9 rounded-lg px-2 text-xs font-bold ${size === preset ? "bg-cyan-400 text-slate-950" : "bg-white/10"}`}>{preset}</button>)}</div>
