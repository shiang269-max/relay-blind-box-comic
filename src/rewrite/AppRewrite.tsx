...
function AppRewrite() {
  ...
  const { room, game, players, isHost, loading, error, start, submit, restart } = useRoom({ roomId, playerId, playerName, createIfMissing, enabled: connected });
  ...
  const joinRoom = useCallback((nextRoom: string) => { const normalized = nextRoom.trim().toUpperCase(); if (!normalized) return; setRoomTarget({ id: normalized, createIfMissing: false }); setConnected(true); setScreen("game"); window.location.hash = normalized; }, []);
  const createNewRoom = useCallback(() => { const next = generateRoomId(); setRoomTarget({ id: next, createIfMissing: true }); setConnected(true); setScreen("lobby"); window.location.hash = next; }, []);
  const handleRestart = useCallback(async () => { await restart(); setConnected(false); setScreen("lobby"); setRoomTarget({ id: generateRoomId(), createIfMissing: true }); window.history.replaceState(null, "", window.location.pathname + window.location.search); }, [restart]);
  ...
  if (screen === "game" && room && game && (game.phase === "playing" || game.phase === "review")) return <GameRouter room={room} game={game} players={players} submit={submit} roomId={roomId} playerId={playerId} playerName={playerName} onLeaveGame={handleRestart} />;
