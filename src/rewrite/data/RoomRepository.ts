...
import { cancelClearVote, castClearVote, getHostId, hasClearVotePassed, nextRoundState, orderPlayers, recoverMissingCurrentPlayer, requestClearVote } from "../game/GameRules";
...
export async function startGame(roomId: string, playerId: string, map: MapType, mode: GameModeId = getDefaultGameMode()): Promise<string> {
  ...
  const roomResult = await runTransaction(roomRef(roomId), (raw: RoomState | null) => {
    if (!raw) return raw;
    const players = activePlayers(normalizePlayers(raw.players), createdAt);
    if (!players[playerId] || raw.currentGameId != null) return raw;
    const ids = orderPlayers(players).map(p => p.id);
    ...
    return { ...raw, players, currentGameId: gameId, lobby: { selectedMode: mode, selectedMap: map } } satisfies RoomState;
  }, { applyLocally: false });
