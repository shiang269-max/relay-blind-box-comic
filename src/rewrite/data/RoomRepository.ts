...
export async function upsertPlayer(roomId: string, player: Player): Promise<void> {
  await runTransaction(roomRef(roomId), (current: RoomState | null) => {
    ...
    return { ...current, players: { ...players, [player.id]: { ...player, joinedAt: existing?.joinedAt ?? player.joinedAt } }, currentGameId, lobby, createdAt: current.createdAt ?? Date.now() } satisfies RoomState;
  }, { applyLocally: false });
}
