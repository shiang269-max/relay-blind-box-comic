...
export async function upsertPlayer(roomId: string, player: Player): Promise<void> {
  await runTransaction(roomRef(roomId), (current: RoomState | null) => {
    ...
    return { ...current, players: { ...current.players, [player.id]: { ...player, joinedAt: current.players[player.id]?.joinedAt ?? player.joinedAt } } } satisfies RoomState;
  }, { applyLocally: false });
}
...
export async function leaveRoom(roomId: string, playerId: string): Promise<boolean> {
  ...
  const result = await runTransaction(roomRef(roomId), (current: RoomState | null) => {
    ...
    const players = { ...current.players }; delete players[playerId];
    if (Object.keys(players).length === 0) return null;
    return { ...current, players } satisfies RoomState;
  }, { applyLocally: false });
  if (result.committed && room?.currentGameId) await recoverMissingCurrentPlayerTurn(roomId, room.currentGameId);
  return result.committed;
}

export async function recoverMissingCurrentPlayerTurn(roomId: string, gameId: string): Promise<boolean> {
  const [roomSnapshot, gameSnapshot] = await Promise.all([get(roomRef(roomId)), get(gameRef(gameId))]);
  const room = roomSnapshot.val() as RoomState | null;
  ...
  const result = await runTransaction(gameRef(gameId), (current: GameState | null) => current && current.roomId === roomId ? recoverMissingCurrentPlayer(current, activePlayerIds) ?? undefined : undefined, { applyLocally: false });
  return result.committed;
}
...
export async function voteToClearPage(gameId: string, playerId: string): Promise<boolean> {
  const result = await runTransaction(ref(db), (root: unknown) => {
    ...
    const game = currentRoot.games?.[gameId]; if (!game) return;
    const voted = castClearVote(game, playerId); if (!voted) return;
    const approved = hasClearVotePassed(voted);
    const nextGame = approved ? { ...voted, clearVote: null } : voted;
    const relayPages = currentRoot.relayPages ?? {};
    const gamePages = relayPages[gameId] ?? {};
    const nextPages = approved ? { ...gamePages, [String(game.currentTurn)]: null } : gamePages;
    return { ...currentRoot, games: { ...(currentRoot.games ?? {}), [gameId]: nextGame }, relayPages: { ...relayPages, [gameId]: nextPages } };
  }, { applyLocally: false });
  return result.committed;
}
